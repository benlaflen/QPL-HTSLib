...
A copy of QPL is included in the library for convenience.
IAA is not required to use QPL-HTSLib, but an Intel processor is.
It is known to be compatible with gcc, g++ and clang.

QPL-HTSlib implements a generalized BAM index, with file extension `.csi`
(coordinate-sorted index). The QPL-HTSlib file reader first looks for the new index
...
Installation follows the same process as for standard HTSLib:
